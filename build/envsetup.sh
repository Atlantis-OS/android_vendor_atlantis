function __print_atlantis_functions_help() {
cat <<EOF
Additional AtlantisOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- atlantisgerrit:   A Git wrapper that fetches/pushes patch from/to AtlantisOS Gerrit Review.
- atlantisrebase:   Rebase a Gerrit change and push it again.
- atlantisremote:   Add git remote for AtlantisOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for AtlantisOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Atlantis model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch atlantis_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/atlantis-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.atlantis.device | grep -q "$ATLANTIS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ATLANTIS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function atlantisremote()
{
    ...
    git remote rm atlantis 2> /dev/null
    ...
    then
        ...
        local PFX="AtlantisOS/"
    else
        local PROJECT=$REMOTE
    fi

    local ATLANTIS_USER=$(git config --get review.review.atlantisos.org.username)
    if [ -z "$ATLANTIS_USER" ]
    then
        git remote add atlantis ssh://review.atlantisos.org:29418/$PFX$PROJECT
    else
        git remote add atlantis ssh://$ATLANTIS_USER@review.atlantisos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'atlantis' created"
}
...
function githubremote()
{
    ...
    git remote add github http://github.com/Project-Atlantis/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.atlantis.device | grep -q "$ATLANTIS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        atlantisremote
        git push atlantis HEAD:refs/heads/'$1'
    fi
    '
}

function atlantisgerrit() {
    ...
    local user=`git config --get review.review.atlantisos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "atlantisgerrit" ]; then
...
function atlantisrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AtlantisOS Gerrit Rebase Usage: "
        echo "      atlantisrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.atlantisos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.atlantis.device | grep -q "$ATLANTIS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ATLANTIS_FIXUP_COMMON_OUT ]; then
