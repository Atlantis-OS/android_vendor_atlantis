...
func init() {
	android.RegisterModuleType("atlantis_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = atlantisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = atlantisExpandVariables(ctx, dep_file)
	...
	cmd := atlantisExpandVariables(ctx, String(g.properties.Cmd))
